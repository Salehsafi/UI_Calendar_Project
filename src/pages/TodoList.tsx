...
const TodoList = () => {
  ...
  useEffect(() => {
    ...
    // Dispatch event for other components (like Progress) to know todos changed
      ...
      <Card className={`w-full max-w-2xl shadow-lg ${isLightMode ? 'light-mode-shadow' : ''}`}>
        <CardHeader className="pb-2">
          <div className="flex items-center mb-3">
            ...
            <div>
              <CardTitle className="text-2xl font-bold text-purple-500">Tasks Manager</CardTitle>
              <CardDescription>Organize your daily priorities</CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            {sortedTodos.length === 0 ? (
              <div className="p-8 text-center flex flex-col items-center justify-center space-y-4 py-12">
                <div className="bg-purple-100 dark:bg-purple-900/30 p-4 rounded-full">
                  <ListTodo className="h-12 w-12 text-purple-500" />
                </div>
                <div>
                  <h3 className="text-lg font-medium mb-1">No tasks yet</h3>
                  <p className="text-muted-foreground text-sm max-w-xs mx-auto">
                    Add your first task using the field above and start being productive today!
                  </p>
                </div>
                <Button 
              ...
              <div className="divide-y divide-purple-500/10">
                {sortedTodos.map((todo) => (
                  ...
                  >
                    ...
                    >
                      <CheckCircle2 className={`h-5 w-5 ${todo.completed ? "fill-purple-200 dark:fill-purple-900/50" : ""}`} />
                    </Button>
                    <span 
                      className={`${
                        todo.completed 
